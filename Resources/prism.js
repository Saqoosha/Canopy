/* Prism.js v1.29.0 | MIT License | https://prismjs.com/ */
...
;(function() {
  ...
  function highlightBlock(code) {
    ...
    if (!match) return;
    var lang = match[1];
    var aliases = { js: 'javascript', ts: 'typescript', sh: 'bash', shell: 'bash', py: 'python', rb: 'ruby', yml: 'yaml', rs: 'rust', md: 'markdown', html: 'markup', xml: 'markup', dockerfile: 'docker' };
    lang = aliases[lang] || lang;
    if (!Prism.languages[lang]) return;

    var text = code.textContent;
    if (contentMap.get(code) === text) return; // No change
    contentMap.set(code, text);
    code.innerHTML = Prism.highlight(text, Prism.languages[lang], lang);
  }

  function highlightAll() {
    document.querySelectorAll('pre code[class*="language-"]').forEach(highlightBlock);
  }
  ...
  // Debounced MutationObserver for streaming
  var pending = false;
