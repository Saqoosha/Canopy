...
final class ShimProcess: NSObject, WKScriptMessageHandler, @unchecked Sendable {
    ...
    /// the numbers under it. Nil while unresolved, and for a `.host` key the
    /// registry has not yet folded into an email.
    var rateLimitEmail: String? {
        guard case .resolved(let key) = rateLimitBinding else { return nil }
        switch SharedRateLimitData.shared.canonicalKey(for: key) {
    ...
    /// **Called at the START of a request, never at a turn's end, and that
    /// is the whole correctness argument.** A prompt-cache entry's lifetime runs
    ...
    ///
    /// Stamping at submission is conservative in the safe direction: an
    ...
    /// to prevent. `trackCacheWindow` closes most of that gap by stamping
    /// again at each request's `message_start`.
    private func noteApiActivity() {
        keepAliveGate.noteActivity(at: Date())
    }

    /// What one webview-bound frame says about the prompt-cache window.
    /// Pure, so the probe can pin the frame selection without a shim.
    enum CacheWindowReading: Equatable {
        /// A main-conversation API request just started (`message_start`).
        case requestStarted
        /// A main-conversation `assistant` frame carried usage; the payload
        /// is `KeepAliveGate.observedTTL(inUsage:)`'s reading, nil when the
        /// request wrote nothing to the cache.
        case usage(KeepAliveGate.ObservedCacheTTL?)
    }

    /// Classify one frame for `trackCacheWindow`. Nil for anything that
    /// says nothing about the window.
    ///
    /// The TTL is read off main-conversation `assistant` frames only: a
    /// subagent's `usage` describes the subagent's own prefix, which can be
    /// on a different window from the conversation this defends.
    /// `message_start` needs no such guard — the CLI hardcodes
    /// `parent_tool_use_id: null` on every `stream_event`, and subagent
    /// traffic is routed out as `assistant` / `user` frames instead (see the
    /// guard's comment in `extractStatusData`).
    static func cacheWindowReading(_ message: [String: Any]) -> CacheWindowReading? {
        ...
        case "assistant":
            ...
            return .usage(KeepAliveGate.observedTTL(inUsage: usage))
        default:
            return nil
        }
    }

    /// Feed `KeepAliveGate` the two things the wire says about the cache
    /// window: which TTL the API granted, and when each request went out.
    ///
    /// Deliberately its own tracker rather than a branch of
    /// `extractStatusData`, which returns early without a `statusBarData`
    /// — the gate must not depend on the status bar being mounted. It also
    /// runs AHEAD of `consumeKeepAliveTraffic`, so a refresh's own frames
    /// feed it: the reasoning is at the pipeline site.
    ///
    /// The per-request stamp is on `message_start`, which the CLI emits
    /// once per API request within a turn. `noteApiActivity` at submission
    /// already covers the turn's FIRST request; a long agentic turn issues
    /// many more, and the entry's lifetime runs from the last of them.
    /// Without this stamp a 30-minute turn was declared stale 30 minutes
    /// before it was, costing one refresh per long turn. The frame arrives
    /// one time-to-first-token after the request started, which puts the
    /// estimate late by seconds against a five-minute margin.
    private func trackCacheWindow(_ message: [String: Any]) {
        switch Self.cacheWindowReading(message) {
        case .requestStarted:
            // `consumeRecapTraffic` has no `stream_event` case, so a recap
        ...
        case .usage(let reading):
            let before = keepAliveGate.observedTTL
            keepAliveGate.noteObservedTTL(reading)
            if let after = keepAliveGate.observedTTL, after != before {
                // `notice`, because a 5m reading switches the refresh off
                // for the rest of this shim and every later decline is
                // logged at `debug` by the coordinator; this line is the one
                // archived trace of why.
                logger.notice("keep-alive \(self.keepAliveLogLabel, privacy: .public): cache window measured as \(after == .oneHour ? "1h" : "5m", privacy: .public)\(after == .fiveMinutes ? " — refresh off until a 1h reading arrives" : "", privacy: .public)")
            }
        case nil:
            break
        }
    ...
    ///
    /// Exists so the swallow — the part of this feature that hides a turn
    /// from Canopy's own trackers, and the part whose one shipped bug was a
    /// wrong swallow — is decidable by a pure function the probe can reach.
    /// The earlier claim that it could not be was wrong: the decision reads
    /// the message and two Bools and nothing else, which is exactly the
    /// shape `strippingRecapFromReplay` already has. Reverting the
    /// `system/init` swallow used to leave the whole suite green; it is now
    /// one red assertion.
    ///
    /// "Swallow" here means **skip the trackers**. Every swallowed frame is
    /// still forwarded to the webview — see `consumeKeepAliveTraffic` for why
    /// that changed and what hides the turn on screen instead.
    enum KeepAliveDisposition: Equatable {
        /// Not ours, or nothing in flight. The message continues down the
        /// pipeline untouched.
        case passThrough
        /// Skip the trackers, changing no state.
        case swallow
        /// Skip the trackers, and record that the CLI has acknowledged our
        /// injection.
        case swallowMarkingEcho
        /// Skip the trackers and end the flight. `refreshed` is false when
        /// the turn reported an error, which is NOT a refresh — see the
        /// enum's use in `consumeKeepAliveTraffic`.
    ...
    static func keepAliveDisposition(
        ...
        // not. Accept both so a protocol tweak degrades to "the trackers
        // count our turn" rather than "latched forever".
        ...
        case "command_lifecycle":
            // Emitted twice at the start of the turn. Ours, so the trackers
            // must not see it; the webview gets it anyway and renders
            // nothing for it that the hide script does not cover (measured
            // on 2.1.268 — no chip appeared).
            return .swallow
        ...
        case "system" where ioMsg["subtype"] as? String == "init":
            ...
            // which at the time was DROPPED — so the pane spun forever with
            // no error and no recovery. The same failure
            // `consumeRecapTraffic` documents for the recap, rediscovered by
            // not believing it applied here.
            //
            // Since the swallow stopped dropping frames from the webview,
            // init and result both reach it and the busy flag clears on its
            // own — measured on 2.1.268, with the pane idle. The disposition
            // is still `.swallow` because the trackers must not see an init
            // for a turn the user did not start.
            //
        ...
        case "assistant", "stream_event":
            ...
            // swallowing it would hide somebody else's reply from the
            // trackers.
            return echoSeen ? .swallow : .passThrough

        case "result":
            // Same positive-evidence rule as the recap's, and for the same
            // shape of failure: claiming the first `result` unconditionally
            // eats the next REAL turn's when our injection was dropped. The
            // webview still gets that `result` now, so its busy flag
            // clears; what sticks is the trackers' view — `isWorking` never
            // falls, so the dot and the pad breathe "working" forever, every
            // gate reports "session busy", and no completion notification
            // fires.
    ...
    ///
    /// **Why the trackers never see it.** The pipeline has a later slot
    /// (`isCanopyOwnedResponse`) that hides a message from the webview
    /// while letting the trackers see it, and using it here looks strictly
    /// better — the `result` carries real usage the context meter wants,
    /// unlike the recap's zeroed one. It is wrong for three separate
    /// reasons, each of which fires overnight on a machine nobody is at.
    /// `trackWorkingState` would flip `isWorking`, so every pane's sidebar
    /// dot and MacroPad key would breathe "working" once an hour all night.
    /// Its `result` branch calls `postTaskCompletedNotification()`, so each
    /// refresh would raise a user notification. And a turn finishing is
    /// what marks a session unread, so by morning the pad would be lit
    /// green across every pane with nothing behind it. What that costs
    /// instead is one tick of context-meter staleness — the meter misses
    /// the ~40 tokens the refresh itself added, until the next real turn's
    /// `result` supersedes it. `rate_limit_event` is a separate io_message
    ...
    private func consumeKeepAliveTraffic(_ message: [String: Any]) -> Bool {
        ...
        // what reached the trackers. `notice`, not `info`: this
        ...
        case .completeFlight(let refreshed):
            ...
            guard refreshed else {
                ...
                // `isWorking` never rose because every keep-alive frame
                // skips `trackWorkingState` — so a rolled-back
    ...
    /// needs.
    ///
    /// What the probe below actually does, since only one of the three is
    /// resolved: `extensionPath` is globbed on the remote (its directory name
    /// carries a version, so this side must not pin it) and tested only for
    /// being non-empty; `shim` is a hardcoded path tested with `-f`; `settings`
    /// is composed and never tested at all.
    ...
    /// Read once, at first access. An env var cannot change while the process
    /// runs, so a computed property would re-read it on every spawn — the same
    /// hot-path mistake `CANOPY_KEEPALIVE_MINUTES` already paid for.
    static let spikeRemoteShimEnabled: Bool =
        ProcessInfo.processInfo.environment["CANOPY_SPIKE_REMOTE_SHIM"] == "1"

    // No host validator here on purpose: the call site uses
    // `RemoteSessionHistory.isSpawnableHost`. A stricter one was written here
    // and deleted — it rejected forms ssh accepts (`fe80::1%en0`), which that
    // function's own doc predicts of any tighter pattern.

    private static var spikeRemotePathCache: [String: SpikeRemotePaths] = [:]

    /// Ask the remote where its Canopy lives. Cached per host.
    ///
    /// SPIKE-grade in three ways the design memo argues at length and this does
    /// not repeat: it blocks the main actor for a round trip with no hard
    /// deadline, only successes are cached (so a bad host pays that block on
    /// every spawn), and nothing ever invalidates an entry.
    ///
    ...
    static func spikeRemotePaths(host: String) -> SpikeRemotePaths? {
        ...
        let probe = """
        set -u
        SUPPORT="$HOME/Library/Application Support/Canopy"
        EXT=$(ls -d "$SUPPORT/extensions/anthropic.claude-code-"* 2>/dev/null | sort -V | tail -1)
        SHIM=/Applications/Canopy.app/Contents/Resources/vscode-shim/index.js
        [ -n "$EXT" ] || exit 3
        [ -f "$SHIM" ] || exit 4
        echo "SHIM=$SHIM"
        echo "EXT=$EXT"
        echo "SETTINGS=$SUPPORT/settings.json"
        """
        ...
        do {
            ...
            logger.error("SPIKE: ssh probe stdin write failed: \(error.localizedDescription, privacy: .public)")
            proc.terminate()
            return nil
        }
        ...
        // Status and decode are separate guards so a decode failure cannot be
        ...
        guard let text = String(data: data, encoding: .utf8) else {
            logger.error("SPIKE: ssh probe output was not valid UTF-8")
            return nil
        }
        ...
        for line in text.split(separator: "\n") {
            ...
            found[String(line[line.startIndex..<eq])] = String(line[line.index(after: eq)...])
        }
        guard let shim = found["SHIM"], let ext = found["EXT"], let settings = found["SETTINGS"] else {
            logger.error("SPIKE: ssh probe returned no usable paths")
            return nil
        }
        let paths = SpikeRemotePaths(shim: shim, extensionPath: ext, settings: settings)
        spikeRemotePathCache[host] = paths
        logger.notice("SPIKE: resolved remote Canopy on \(host, privacy: .public)")
        return paths
    }

    /// Build the ssh argv that runs the remote shim.
    ///
    /// Every dynamic value is single-quoted, because ssh flattens its remote
    /// arguments into one string for the remote login shell — the same
    ...
    /// spawn's argv. What resumes a remote session is `data-initial-session` →
    /// the extension's own precheck, which finds the transcript because on the
    /// remote it is local.
    ///
    /// **Nothing in the local environment reaches the remote CLI**, because ssh
    /// forwards none of it and the wrapper that used to serialize it is not in
    /// play. Model and effort are dropped silently; so is
    /// `CLAUDE_CODE_DISABLE_1M_CONTEXT`, so a non-1M Opus selection runs at 1M
    /// over there; so is a configured custom API provider, which is the one that
    /// matters — the conversation goes to an endpoint the user did not choose,
    /// with no log line and nothing visible. This branch does not yet refuse
    /// that combination.
    static func spikeSSHArguments(
        ...
        var remote = "exec node \(q(paths.shim))"
            + " --extension-path \(q(paths.extensionPath))"
            + " --cwd \(q(cwd))"
            + " --settings-path \(q(paths.settings))"
            + " --permission-mode \(q(permissionMode.rawValue))"
    ...
    //
    // Without it the SPIKE mark above swallows `start()` in Xcode's jump bar.
    ...
    func start() -> Bool {
        ...
        // local. Measured against studio by resuming a session whose project
        // directory does not exist here — the full transcript rendered.
        //
        // What moves is what the EXTENSION reads. `ShimProcess` stays here, so
        // what IT reads stays local — `handleOpenFile` and the background-task
        // JSONL scan are not fixed by this branch.
        //
        // With the flag absent nothing below runs, so the two routes A/B
        // against one build. Everything else — the deferred behavioural gaps,
        // the measurements, the wrong turns — is in
        ...
        if let remote = remoteHost, Self.spikeRemoteShimEnabled {
            ...
            guard let paths = Self.spikeRemotePaths(host: remote) else {
                logger.error("SPIKE: cannot resolve remote shim paths on \(remote, privacy: .public)")
                showErrorInWebView(
                    "SPIKE remote shim: could not resolve Canopy paths on \(remote). "
                        + "Is Canopy installed there?"
                )
                return false
            }
            proc.executableURL = URL(fileURLWithPath: "/usr/bin/ssh")
            proc.arguments = Self.spikeSSHArguments(
                host: remote,
                paths: paths,
            ...
            // The wrapper must NOT be in play here. In spike mode the CLI is
            // spawned by the REMOTE extension as an ordinary local child; a
            // surviving `CANOPY_SSH_HOST` would arm `vscode-shim/index.js`'s
            // fs/spawn patches and make the remote shim ssh back to itself.
        ...
        do {
            ...
            // Names what was actually spawned; in spike mode that is ssh, and
            // the thrown error is about ssh rather than about node.
            let what = (remoteHost != nil && Self.spikeRemoteShimEnabled) ? "ssh" : "Node.js"
            showErrorInWebView("Failed to start \(what): \(error.localizedDescription)")
            self.process = nil
            self.stdinPipe = nil
            self.stdoutPipe = nil
            self.stderrPipe = nil
    ...
    private func handleWebviewMessage(_ incoming: [String: Any], sender: (any MirrorSink)?, isPrimary: Bool) {
           ...
           ioMsg["type"] as? String == "user"
        {
            ...
            // restarts here — the turn's first request; `trackCacheWindow`
            // stamps the rest of them at `message_start`, and nothing stamps
            // at a turn's END (`noteApiActivity` carries the reason why).
            // Stamping here also
    ...
    private func patchAuthIfNeeded(_ message: [String: Any]) -> [String: Any] {
           ...
           var state = request["state"] as? [String: Any]
        {
            ...
            // SPIKE ONLY. A remote shim cannot read a keychain over SSH, so the
            // extension omits authStatus from every update_state and the
            // webview's `state.authStatus ?? null` wipes the init_response
            // bootstrap — the pane falls to the login screen. The Keychain read
            // here happens locally, in Canopy's GUI process, which is the one
            // context that can do it.
            //
            // Two costs, both real inside spike mode rather than deferred by the
            // scoping. `/login` cannot reach the login screen, because the login
            // flow's own signal is an update_state with authStatus absent —
            // exactly this trigger. And this shows the LOCAL account while the
            // remote CLI authenticates as its own, so two machines on different
            // accounts display the wrong one; a remote logout is overwritten
            // here too, since on this wire it is indistinguishable from "could
            // not read a keychain". The shipping design replaces this with
            // `claude auth status --json` on the remote.
    ...
    /// fetches them; a Mac only once its attach says it can (`"images": true`, 2.47+), since an older one
    ...
    /// The long edge a replayed inline image is re-encoded to for a remote client. The API downsizes an image to
    /// about this before the model sees it, so the mirror shows what the model saw.
    static let mirrorInlineImageMaxPixelSize = 1568

    /// A base64 image re-encoded as JPEG at `mirrorInlineImageMaxPixelSize`, or nil when that does not shrink it.
    nonisolated static func shrinkMirrorImage(base64: String) -> String? {
        guard let bytes = Data(base64Encoded: base64),
              let smaller = RosterImageUploader.thumbnail(from: bytes, maxPixelSize: mirrorInlineImageMaxPixelSize, quality: 0.8),
              smaller.count < bytes.count
        else { return nil }
    ...
    /// - every base64 image still inline (a screenshot tool's, one the user pasted), passed through `shrinkImage`.
    ///   These stay base64 because the extension draws nothing for any other source type. Read images are
    ///   `deferringReadImagesForMirror`'s, and are URLs by the time this runs.
    ///
    ...
    private func resetActivityState() {
        ...
        // claims `command_lifecycle` and `system/init` on the strength of
        // the latch alone, with no echo required. A latch that survived a
        // reconnect would read the reconnected session's first real turn as
        // ours — its `result` arriving with no echo logs a spurious
        // `.error`, and had `echoSeen` survived too, the whole turn would be
        // invisible to the trackers. The 120 s watchdog bounds that window
        // but does not close it, and a reconnect inside it is the normal
        // case for SSH remote.
        endKeepAliveFlight()
